...
# 3. 실제 값과 예측 확률을 하나로 합쳐서 보기 (가독성 향상)
...
-   **규제 선형 모델 (Regularization):** Ridge(L2), Lasso(L1), ElasticNet의 차이와 페널티 항을 이해해야 합니다.
    ...
    -   Ridge (L2) : 모든 변수를 유지하되, 영향력을 줄입니다. 변수 간 상관관계가 높을 때 유리합니다.(계수의 제곱합에 패널티)
    -   Lasso (L1) : 중요하지 않은 변수의 계수를 0으로 만듭니다. 자동으로 변수를 선택하는 효과가 있습니다.(계수의 절대값 합에 패널티)
    -   ElasticNet : Ridge와 Lasso의 장점을 합친 모델입니다. 변수가 많고 서로 연관되어 있을 때 가장 강력합니다.(L1 + L2 혼합)
...
# 6. 결과 확인
print(en_model_final)

# 7. 최적의 모델 결과 확인
print(en_model_final$bestTune)
...
# 5. 최적의 파라미터 조합 확인 (어느 정도의 규제가 채택되었는지 확인)
...
# 최종 성능 평가
postResample(pred = predictions, obs = test_data_clean$sale_price)

imp_xgb <- varImp(xgb_model, scale = TRUE)
...
#실루엣 계수는 1에 가까울수록 "내 그룹 안에는 똘똘 뭉쳐있고, 다른 그룹과는 멀리 떨어져 있다"는 뜻입니다. 따라서 그래프상에서 세로축(Average Silhouette Width) 값이 가장 높게 솟아오른 점의 K값이 수학적으로 가장 완벽한 군집 개수
```
...
# 결과를 출력했을 때 노이즈(0번)가 너무 많다면 반경(eps)을 조금씩 넓혀주거나(0.6, 0.7), 최소 데이터 개수(minPts) 기준을 낮추어 밀도 조건을 완화해 가며 최적의 형태를 찾을 수 있습니다.
```
```{r}
###연관 규칙 분석 (Association Rule), Apriori 알고리즘
...
# support = 0.1: 주택 데이터의 최소 10% 이상에서 발견되는 흔한 조합만 찾겠다는 의미입니다. 데이터셋이 크다면 이 값을 낮추어 희귀하지만 가치 있는 규칙을 찾을 수 있습니다.
